...
The three survival conversations highlight the effect of client-side compaction limits. `maf-compact` failed with an upstream HTTP 502 error after its 256,000 token `MaxContextWindowTokens` limit fired, whereas `comprexy` completed one additional prompt beyond each baseline failure (the harness default `survived_baseline_failure` margin). Comparing only the common completed prefix (prompts 1 through X-1), `comprexy` saved 496,776 tokens (26.4%) in edge-case-noisy, but ran 1,281,863 tokens (35.3%) over in long-planning and 208,258 tokens (4.0%) over in short-deep. Completing past the baseline failure point is the intended harness outcome for these runs.
