...
| **Application** | Conversation identity, prepare/complete chat, soft-budget decisions, context rebuild, Inline wrap-up, token metrics / telemetry query facade, conversation retrieval (RAG) query facade |
...
2. **Outbound rewrite** — model-facing `tools` = bound Virtual tools from the registry (`comprexy_read_file_manifest`, `comprexy_read_file_range`, `comprexy_read_file_search`, `comprexy_dir_list`, `comprexy_shell`) + `comprexy_get_current_conversation_id` + full-schema passthrough of client tools that Virtual Tools does **not** replace (mutates like `write`/`edit`, MCP/browser/`NON_FILE`, and unbound `OTHER_FILE`/`FILE_METADATA`). Only replaced backends (`FILE_READ_RAW` / `FILE_SEARCH_BACKEND` / `DIRECTORY_LIST_BACKEND` / `SHELL_BACKEND` and binding primaries) are hidden from the model catalog. No compact index and no `get_tool_definition`. Upstream **messages** always come from the stored IR-side transcript rebuild (WM optional); client wire history is never used as the model transcript.
...
Configuration: [`SETTINGS.md`](SETTINGS.md#toolschema). Design notes: `internal/plans/virtual-tools.md`, Shell family: `internal/plans/shell-ir.md`.
