...
- Live outgoing context also wire-omits older identical **failed** file-edit tool results (path + `old_string` last-wins) when `DedupeDuplicateFailedEdits` is on — same shape as duplicate file-read omit; does not `MarkFoldedInto`.
...
| Duplicate failed edits | `DuplicateFailedEditDeduper` + `FileMutationClassifier` / `FileMutationEditIndexer` (live chat wire omit, when enabled) |
