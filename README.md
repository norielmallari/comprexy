...
| Tool denylist | `ToolSchema:ExcludeFromModelTools` hides listed client tools from the model (case-insensitive; stock defaults include Cursor UX tools plus Kilo `agent_manager` / `background_process` / `task` / `kilo_local_recall` / `agent_manager_models`), rejects calls locally, and swallows inbound orphans |
